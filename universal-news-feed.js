document.addEventListener("DOMContentLoaded", function () {
  ...
  const allNews = fashionNewsData.posts || [];
  const PLACEHOLDER_URLS = fashionNewsData.placeholders || [];
  const LOAD_MORE_ENABLED = !!parseInt(fashionNewsData.load_more_enabled, 10);
  const ITEMS_PER_PAGE = parseInt(fashionNewsData.items_per_page, 10) || 8;
  let currentPage = 0;
  ...
  function renderItems(itemsToRender) {
    ...
    itemsToRender.forEach((news) => {
      ...
      // --- NEW LOGIC: We will track if this item is using a placeholder ---
      ...
      if (!imageUrl && PLACEHOLDER_URLS.length > 0) {
        ...
        // --- NEW LOGIC: If we used a placeholder, set the flag to true ---
        isPlaceholder = true;
      }
      ...
      let imageHtml = "";
      if (imageUrl) {
        // --- NEW LOGIC: Add the 'is-placeholder' class if the flag is true ---
