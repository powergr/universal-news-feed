...
class Universal_News_Feed_Plugin {
    ...
    public function load_textdomain() { load_plugin_textdomain('universal-news-feed'); }
    ...
    public function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <div class="notice notice-info inline" style="display:flex; justify-content: space-between; align-items: center; padding: 12px; margin-top: 15px;"><div><p style="margin: 0 0 10px 0;"><strong><?php _e('Active Shortcode:', 'universal-news-feed'); ?></strong> <code>[<?php echo esc_html($this->shortcode_tag); ?>]</code></p><p style="margin: 0 0 10px 0;"><strong><?php _e('Filter by source:', 'universal-news-feed'); ?></strong> <code>[<?php echo esc_html($this->shortcode_tag); ?> source="Source Name"]</code> <span style="color:#666;"><?php _e('— shows only that source. Use the exact Source Name from the Feeds tab below; comma-separate multiple names, e.g.', 'universal-news-feed'); ?> <code>source="Source One, Source Two"</code>. <?php _e('Add several shortcodes with different source values to build separate sections on one page.', 'universal-news-feed'); ?></span></p><p style="margin: 0;"><strong><?php _e('Last Update:', 'universal-news-feed'); ?></strong> <?php $last_update_gmt = get_option('lfn_last_update_timestamp'); echo $last_update_gmt ? esc_html(get_date_from_gmt(date('Y-m-d H:i:s', $last_update_gmt), get_option('date_format') . ' ' . get_option('time_format'))) : __('Never', 'universal-news-feed'); ?> <em style="display: block; font-size: 12px; color: #666;"><?php
                /* translators: %s: URL of the General Settings screen, where the site timezone can be changed. */
                printf(__('Time is based on your <a href="%s">WordPress timezone setting</a>.', 'universal-news-feed'), esc_url(admin_url('options-general.php')));
    ...
    public function render_load_more_field() { $options = get_option('lfn_settings'); $load_more = isset($options['load_more_enabled']) ? $options['load_more_enabled'] : 1; echo '<input type="checkbox" name="lfn_settings[load_more_enabled]" value="1"' . checked(1, $load_more, false) . '> ' . __('Enable "Load More" button', 'universal-news-feed'); }
    ...
    public function render_layout_field() { $options = get_option('lfn_settings'); $layout = isset($options['layout']) ? $options['layout'] : 'list'; echo '<select name="lfn_settings[layout]"><option value="list" ' . selected($layout, 'list', false) . '>' . __('List', 'universal-news-feed') . '</option><option value="grid" ' . selected($layout, 'grid', false) . '>' . __('Grid', 'universal-news-feed') . '</option></select>'; }
    ...
    public function render_custom_css_field() { $options = get_option('lfn_settings'); $css = isset($options['custom_css']) ? $options['custom_css'] : ''; echo '<textarea name="lfn_settings[custom_css]" rows="8" cols="50" class="large-text code">' . esc_textarea($css) . '</textarea><p class="description"><strong>' . __('How to use Custom CSS:', 'universal-news-feed') . '</strong><br>' . __('Key Classes:', 'universal-news-feed') . ' <code>.lfn-container</code>, <code>.news-item</code>, <code>.news-title a</code></p>'; }
    public function render_feeds_field() { $options = get_option('lfn_settings'); ?><div id="lfn-feeds-container"><p class="description"><?php _e('Add the name and URL for each RSS feed source.', 'universal-news-feed'); ?></p><?php $feeds = isset($options['rss_feeds']) ? $options['rss_feeds'] : []; if(!empty($feeds)){foreach($feeds as $index=>$feed){?><div class="lfn-feed-row"><input type="text" name="lfn_settings[rss_feeds][<?php echo $index;?>][name]" value="<?php echo esc_attr($feed['name']);?>" placeholder="<?php esc_attr_e('Source Name', 'universal-news-feed');?>" size="30"/><input type="url" name="lfn_settings[rss_feeds][<?php echo $index;?>][url]" value="<?php echo esc_attr($feed['url']);?>" placeholder="<?php esc_attr_e('RSS Feed URL', 'universal-news-feed');?>" size="50"/><button type="button" class="button lfn-remove-feed"><?php _e('Remove', 'universal-news-feed'); ?></button></div><?php }}?></div><button type="button" class="button" id="lfn-add-feed"><?php _e('Add Feed', 'universal-news-feed'); ?></button><?php }
    public function render_placeholders_field() { $options = get_option('lfn_settings'); ?><div id="lfn-placeholders-container"><p class="description"><?php _e('Select images from your Media Library.', 'universal-news-feed'); ?></p><?php $placeholders = isset($options['placeholders']) ? $options['placeholders'] : []; if(!empty($placeholders)){foreach($placeholders as $index=>$url){?><div class="lfn-placeholder-item"><img src="<?php echo esc_url($url);?>"/><input type="hidden" name="lfn_settings[placeholders][]" value="<?php echo esc_url($url);?>"><button type="button" class="button lfn-remove-placeholder"><?php _e('Remove', 'universal-news-feed');?></button></div><?php }}?></div><button type="button" class="button" id="lfn-add-placeholder"><?php _e('Add Placeholder Image', 'universal-news-feed'); ?></button><style> .lfn-feed-row{margin-bottom:10px;} #lfn-placeholders-container{display:flex;flex-wrap:wrap;gap:15px;} .lfn-placeholder-item{position:relative;} .lfn-placeholder-item img{width:100px;height:100px;object-fit:cover;border:1px solid #ddd;} .lfn-placeholder-item button{position:absolute;top:5px;right:5px;} </style><?php }
    public function enqueue_admin_assets($hook) { if ($hook !== 'settings_page_universal_news_feed_settings') { return; } wp_enqueue_media(); wp_enqueue_style('wp-color-picker'); wp_enqueue_script('lfn-admin-script', plugin_dir_url(__FILE__) . 'admin-scripts.js', ['jquery', 'wp-color-picker'], '12.0', true); }
